...
function Exercising({ token, onScreenChange, trainingExerciseId, setTrainingProgress, trainingProgress }) {
    ...
    const addRecordToHistory = () => {
        ...
        setRecordInfo({ weight: '', reps: '', note: '' }); // Очищаємо форму
    };

    const fetchExerciseHistoryData = () => {
        if (exerciseHistory.length === 0) return <HistoryRecord>No added series yet</HistoryRecord>
        return exerciseHistory.map((record, index) => (
    ...
    return (
        <ExerciseFrame>
            ...
            <ControllPanel>
                <FlexItems>
                    <Input style={{ width: '99px' }} placeholder="Reps" value={recordInfo.reps}
                        onChange={e => {
                            setRecordInfo(prev => ({ ...prev, reps: e.target.value }));
                        }}
                    />
                    <Input style={{ width: '99px' }} placeholder="Weight" value={recordInfo.weight}
                        onChange={e => {
                            setRecordInfo(prev => ({ ...prev, weight: e.target.value }))
                        }}
                    />
                    <Button width={'70px'} onClick={() => addRecordToHistory()}>Add</Button>
                </FlexItems>
                <FlexItems>
                    <Input style={{ width: '233px' }} placeholder="Note" value={recordInfo.note}
                        onChange={e => {
                            setRecordInfo(prev => ({ ...prev, note: e.target.value }))
                        }}
                    />
